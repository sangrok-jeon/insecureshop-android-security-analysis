...
상세 보고서: [02-insufficient-url-validation.md](./findings/02-insufficient-url-validation.md)

`WebViewActivity`의 `/web` 분기에서는 deeplink로 전달된 `url` 파라미터가 별도 allowlist 검증 없이 곧바로 `webview.loadUrl()`에 전달되었다. `nox_adb shell am start`로 `naver.com`을 직접 로드해 임의 URL 로딩 가능성을 확인하였다.

### 4.3 Weak Host Validation Check

상세 보고서: [03-weak-host-validation-check.md](./findings/03-weak-host-validation-check.md)

`/webview` 분기에서는 검증이 존재했지만, 실제 host 비교가 아니라 `endsWith("insecureshopapp.com")` 기반 문자열 비교만 수행하고 있었다. 이를 이용해 실제 host가 `naver.com`인 URL로도 검증 우회가 가능함을 확인하였다.
