...
이미지는 취약점별로 분리해 저장하고, 각 섹션의 `증빙 이미지` 항목에 적어둔 파일명 규칙을 그대로 사용하는 것을 기준으로 한다. 이렇게 정리하면 README 수정과 이미지 교체가 쉬워지고, GitHub에서도 구조가 한눈에 보인다.
...
- 증빙 이미지는 취약점당 3~4장으로 제한해 핵심만 남긴다.
