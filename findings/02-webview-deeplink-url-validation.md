...
### 1. `/web`와 `/webview` 경로 처리 코드 확인

![1. `/web`와 `/webview` 경로 처리 코드 확인](../images/02-insufficient-url-validation/02-webview-path-validation-code.png)
...
### 3. `/webview` 분기의 약한 검증 코드 확인

![3. `/webview` 분기의 약한 검증 코드 확인](../images/03-weak-host-validation-check/02-webview-endswith-code.png)
...
![4. `/webview` 분기의 약한 검증 우회 확인](../images/03-weak-host-validation-check/04-webview-bypass-load-naver.png)
